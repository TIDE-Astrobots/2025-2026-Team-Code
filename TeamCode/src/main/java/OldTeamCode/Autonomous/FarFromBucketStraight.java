 ...
 */
//@Config
@Autonomous(name = "Far From Bucket (Straight) V1.1.14")
public class FarFromBucketStraight extends LinearOpMode {
...
//        telemetry = new MultipleTelemetry(telemetry, FtcDashboard.getInstance().getTelemetry());
        ...
        WheelMotors[0].setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        WheelMotors[1].setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        WheelMotors[2].setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        WheelMotors[3].setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        ...
        while(opModeIsActive()) {
             ...
             */
            //1.
            List<List<String>> steps = gameField.getInstructionsList("3-1", "6-1");
            ...
            taskNumber = 0;
            steps = gameField.getInstructionsList("6-1", "6-2");
            telemetry.addData("STEPS", steps);
            telemetry.update();
            resetTask();
            while(true) {
            ...
            //9. Move left 2 inches
            resetTask();
            while(true) {
                if(runOnce) {
                    runOnce = false;
                    moveDirectionInInches("left", 2);
            ...
            while(true) {
                if(runOnce) {
                    runOnce = false;
                    moveDirectionInInches("forward", 11);
                }
...
//                    extendoRight.setTargetPosition(3000);
            ...
            //9. Move backwards 11 inches
            resetTask();
            while(true) {
                if(runOnce) {
                    runOnce = false;
                    moveDirectionInInches("back", 11);
            ...
            //11. Move 14 inches right
            resetTask();
            while(true) {
                if(runOnce) {
                    runOnce = false;
                    moveDirectionInInches("right", 12.75f);
                }
            ...
            //11.5 MOVE 2 INCHES FORWARD
            resetTask();
            while(true) {
                if(runOnce) {
                    runOnce = false;
                    moveDirectionInInches("forward", 2);
                }
            ...
            while(true) {
                ...
                if(armPivot.getCurrentPosition() < -104) {
                    armPivot.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
                    armPivot.setPower(0.05);
                }
                telemetry.addData("Task number:", taskNumber);
                telemetry.addData("Power", armPivot.getPower());
                telemetry.addData("RunModed", armPivot.getMode());
            ...
            //14.5 MOVE 2 INCHES BACK
            resetTask();
            while(true) {
                if(runOnce) {
                    runOnce = false;
                    moveDirectionInInches("back", 2);
                }
            ...
            while(true) {
                if(runOnce) {
                    runOnce = false;
                    moveDirectionInInches("left", 12.75f);
                }
            ...
            while(true) {
                ...
                if(armPivot.getCurrentPosition() < -90) {
